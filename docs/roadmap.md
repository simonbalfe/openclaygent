...
- [x] Fetch ladder — impit (browser TLS) + Crawl4AI-style pruning extraction (`src/tools/extract.ts`), escalating through patchright (direct → +proxy → +solver) to paid Tavily `/extract` (always-live). Mechanism in `architecture.md` (The tools) + `decisions.md` (Fetch ladder)
